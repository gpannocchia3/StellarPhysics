...
with 
\begin{equation}
    \frac{S_{\text{eff}}}{S(0)} = 1+\frac{5kT}{36E_{0}}+\frac{S'(0)}{S(0)}\left(E_{0}+\frac{35}{36}kT\right)+\frac{1}{2}\frac{S''(0)}{S(0)}\left(E_{0}^{2}+\frac{89}{36}kTE_{0}\right)
...
\end{equation*}
\begin{equation*}
    \nu = \der{\ln\langle \sigma v \rangle}{\ln T}\Big|_{T=T_{0}} = -\frac{2}{3}+\frac{\tau(T_{0})}{3}
\end{equation*}
meaning that 
\begin{equation}
    \boxed{
...
\begin{equation*}
    a \sim \frac{1}{n_{i}^{1/3}} \sim \left(\frac{\mu_{i}m_{\text{H}}}{\rho}\right)^{1/3}
\end{equation*}
...
\begin{equation*}
    \rho = \sum_{i}Z_{i}e n_{i}-en_{e} = \sum_{i}\left(-\frac{Z_{i}^{2}e^{2}\Phi\bar{n}_{i}}{kT}\right)-\frac{e^{2}\Phi\bar{n}_{e}}{kT}
\end{equation*}
...
\begin{equation*}
    n = \bar{n}_{e} + \sum_{i}\bar{n}_{i} \quad \text{at}\; \Phi = 0
\end{equation*}
and we introduce the quantity $\chi$ classically defined as 
\begin{equation*}
    \chi = \frac{\sum_{i}Z_{i}^{2}\bar{n}_{i}+\bar{n}_{e}}{n}
\end{equation*}
...
\begin{equation}
    \Phi \approx \frac{Z_{i}e}{r}-\frac{Z_{i}e}{r_{D}}
\end{equation}
...
\par During the star's evolution along the Hayashi track (see Chapter \ref{ch:v}), the star increases its temperature due to the virial theorem, and a radiative core will form at the center and grow in mass as the star evolves. When the radiative core appears, the star is no longer fully convective; this happens when 
\begin{equation*}
    \nabla_{\text{rad}}<\nabla_{\text{ad}}
\end{equation*}
...
\par The energy production for the whole set of reaction we'll be going through in a moment can be shown to amount to $26.7\unit{MeV}$, only decreasing to $26.1\unit{MeV}$ if we consider the energy subtracted by neutrinos. In any case, this amount of energy is still 10 times larger than that produced in any other nuclear reaction process occurring in stars.
...
& \element{Li}{7}{3}+\element{p}{1}{1}\to \element{Be}{8}{4}\to 2\,\element{He}{4}{2}\\
& \\
...
\end{figure}
\par The sharpest amongst my readers have surely noticed that there are some elements which are simultaneously involved in destruction and production processes (e.g., $\element{D}{2}{1}$ or $\element{He}{3}{2}$). These elements are usually called \emph{secondary elements}.
\par It should come as no surprise that the variation of the abundance of element $i$ will depend on all the creation and destruction reactions involving that element\footnote{The general equation is quite long and convoluted, and I see no point into copying it here. The interested reader can refer to \cite{2005essp.book.....S}, §3.17, equation 3.39.}. If you recall how we've defined the reaction rate (\ref{eq:reactionrate}), we can then say that the variation over time of the abundance (or number of particles) of a certain species will be 
\begin{equation*}
...
\end{equation*}
Equilibrium will be reached when $\dot{N}_{\text{D}} = 0$, so when
\begin{equation*}
...
\par It is evident that if the abundance of $\element{D}{2}{1}$ is larger than its abundance at equilibrium, then the destruction process prevails over the production one, and the chemical abundance will tend to evolve quickly towards its equilibrium configuration.
...
\end{align*}
\par It should be noticed that for a typical PopI star chemical composition (i.e., about solar) the global amount of CNO elements is of order of 75\% of the total metallicity
\begin{equation*}
...
\end{align*}
Until temperature rises even higher ($T\approx 25\cdot 10^{6}\unit{K}$) there is a \emph{leakage} of un-burned Neon from the star. Only in very massive MS-stars and during the AGB\footnote{\emph{Asymptotic Giant Branch}.} does a star suffice the necessary conditions to activate the Neon-Sodium cycle
\begin{align*}
    ...
    & \element{Na}{23}{11}+\element{p}{1}{1}\to \element{Mg}{24}{12}^{*} \to \element{Ne}{20}{10}+\element{He}{4}{2}\\
...
\par Note that as temperature climbs up to higher heights\footnote{Poetical delirium.} under conditions such as those found in novae and X-ray bursts, the rate of proton captures can exceed the rate of $\beta$-decay, pushing the burning to the proton drip line.
...
\par The first and fundamental reaction in the He-burning process is the production of $^{12}\text{C}$ from the fusion of three Helium nuclei. This reaction is called the $3\alpha$ reaction. This reaction usually occurs in two separate steps
\begin{align*}
...
\end{equation*}
In a short time ($\sim 10^{-16}\unit{s}$), Berillium decays back into two $\alpha$ particles, thus making the probability of the second reaction extremely low and requiring longer timescales for the reaction to occur. However, as the interior temperature climbs up, the probability of the second reaction increases, due to these combined effects:
\begin{itemize}
...
\end{itemize} 
\par A temperature of about $\sim 10^{8}\unit{K}$ is necessary before $3\alpha$ reactions start producing a sizeable amount of energy. Also, any increase in the density favors incredibly this reaction for 
\begin{equation*}
...
\par In 1954, Fred Hoyle \cite{1954ApJS....1..121H} predicted that in order to achieve the observed abundances of Carbon and Oxygen, the $3\alpha$ reaction must be \emph{resonant}.
...
\end{align*}
but the most notable feature of Helium burning is that the star is now able to produce \emph{free neutrons}. Consider for example the following reaction chain
\begin{align*}
...
\end{align*}
For stars of solar metallicity, in which $Z_{\text{CNO}}\sim 0.01Z_{\odot}$, we'd have, for example, a flux of $7\power{10}{20}\,\text{neutrons}\unit{g}^{-1}$, but temperatures of order $3.0-3.5\power{10}{8}\unit{K}$ are needed. So, during the AGB phase, this chain of neutron production is possible only for $M\gtrsim 3M_{\odot}$ (and dominant for $M\gtrsim5M_{\odot}$). Smaller stars produce free neutrons with a reaction involving Carbon 
\begin{align*}
    & \element{C}{12}{6}+\element{p}{1}{1}\to\element{N}{13}{7}+\gamma\\
    & \element{N}{13}{7}\to \element{C}{13}{6}+e^{+}+\nu_{e}\\
    & \element{C}{13}{7}+\element{He}{4}{2}\to\element{O}{17}{O}^{*}\to\element{O}{16}{8}++\element{n}{1}{0}
\end{align*}
that is completed only at temperatures of order $\approx 8\power{10}{7}\unit{K}$, but not much higher since Helium burning would soon be starting.
...
Moving on to even higher temperatures, it is possible to ignite the combustion of heavier nuclei, such as Carbon. Since a temperature of order $T\sim 7-8\power{10}{8}\unit{K}$ is needed, many stars will never even get that far. The minimum mass to ignite Carbon is said $M_{\text{up}}$ and is defined as such
\begin{equation*}
    M_{\text{up}}=
        \left\{
    \begin{array}{ll}
            ...
            \sim 4M_{\odot} & \text{intermediate metallicity stars,}\;\;Z\sim 10^{-3}
    \end{array}
        \right.
\end{equation*}
...
Up until now we've seen how elements up to Iron are produced all throughout stellar nucleosynthesis, and maybe you've began to wonder how even heavier elements like Silver, Gold and Platinum (if you're into riches) or Technetium, Uranium and Radium (if you're into radioactive stuff) are produced. The answer to all your questions was anticlimactically revealed in the title of this section: \emph{Neutronic captures}.
\begin{figure}[h!]
    \centering
...
\begin{equation}
    \tau_{n} = \frac{1}{N_{n}\langle \sigma(v) \rangle_{A+n}v_{T}}
...
\begin{equation*}
    \sigma_{T}v_{T}\sim 3\power{10}{-17}\unit{cm}^{3}\unit{s}^{-1}
\end{equation*}
\par If we want to make sure that a capture is indeed a s-capture, then we have to require $\tau_{n}\gg \tau_{\beta}$, with $\tau_{\beta}$ and $\tau_{n}$ respectively the timescales for $\beta$-decays and neutronic captures. Since the average timescale for $\beta$-decays in these conditions is $\tau_{\beta}\sim 3\power{10}{8}\unit{s}$, the necessary neutron density is
\begin{equation*}
...
\end{equation*}
\par In what remains of this chapter, we'll try to calculate the equilibrium abundance of (stable) nuclei that are forming due to neutronic captures, as well as explaining particular features in the abundances trend.
...
\end{equation*}
\par If we were to take a look at a plot\footnote{I've yet to find one that is at least half-satisfying. For the time being, try to picture it in your head.} of the neutronic capture cross section as a function of $A$, we'd see that there are regions in which the product $N_{A}\sigma_{A}$ (dropping the bra-kets for convenience) is approximately constant in certain regions, while in others we're observing sharp drops. Why is it so?
...
\par It would be a very sensible question indeed. \par The thing is that the more $\beta$-decays bring nuclei closer to the stability valley, the more neutronic captures become probable, hence, unless the neutron flux gets suddenly cut off, it is possible to capture another neutron, cross the magic number barrier and reach higher atomic weigths than the corresponding s-peak.
\begin{figure}[h!]
    \centering 
