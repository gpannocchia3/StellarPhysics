...
\subsection{Universal Curve of Abundancies}
...
\begin{figure}[h!]
	...
	\caption{Comparison bewteen the recommended abundances by \cite{2021A&A...653A.141A} in present-day solar photosphere and CI chondrites.\\
	Most elements agree well, with the exception of the highly volatile elements, which are depleted in meteorites, and Li, which has been largely destroyed in the Sun due to nuclear burning.}
	\label{fgr:chondrites-photsphere}
\end{figure}
\par Still, if this is actually the case, we may expect to measure a constant ratio between the various abundances of elements. Consider for example Fig.\ref{fgr:universalcurveabundancies}.
...
\begin{figure}[h!]
	...
	\caption{Universal curve of abundancies, normalized on the relative abundance of Silicon. It represents the average value in the galactic disk of chemical abundancies in stars. Credits: Wikipedia, but potentially \cite{2021A&A...653A.141A}.}
	\label{fgr:universalcurveabundancies}
\end{figure}
...
\begin{equation}
	...
	\label{eq:fe/h}
\end{equation}
where we've defined $N_{i} = X_{i}/A_{i}m_{\text{H}}$. From this definition is obvious then that if $[\text{Fe}\,|\,\text{H}] = 0$, the abundance of Iron is equal to that of the Sun. Doing a slight manipulation\footnote{Which makes use of $\text{X}+\text{Y}+\text{Z} = 1$.} we can also write
\begin{equation}
