...
\par As customary in Physics, the correct answer is "It depends." Can we take for granted that the chemical composition we see in the photosphere is the same as the original one?
...
A matter we're going to be most interested in discussing is the provenience of the various (chemical) elements we observe on Earth and all around the Universe and \emph{how} they are produced. For example, we now know that the almost entirety of Hydrogen was produced during the \emph{Primordial Nucleosynthesis} along with a bit of Helium. Metals, on the other hand, are entirely produced by \emph{Stellar Nucleosynthesis}. That's because in the early stages of the formation of the Universe, temperature dropped too fast below the critical threshold that allows the relevant nuclear reactions to produce efficiently Helium and metals. On top of that, light metals (Lithium, Beriullium and Boron) are generally destroyed for the sake of producing lighter elements like Helium (see Chapter \ref{ch:iv}) and the figure below (Fig.\ref{fgr:chondrites-photsphere}), which presents a comparison of chemical composition of the solar photosphere with that of CI chondrites.
\begin{figure}[h!]
	\centering 
...
\end{figure}
\par Still, if this is actually the case, we may expect to measure a constant ratio between the various abundances of elements. Consider for example Fig.\ref{fgr:universalcurveabundances}.
...
\begin{figure}[h!]
	...
	\caption{Universal curve of abundances, normalized on the relative abundance of Silicon. It represents the average value in the galactic disk of chemical abundances in stars. Credits: Wikipedia, but potentially \cite{2021A&A...653A.141A}.}
...
\end{equation*}
If, for whatever reason, we want to determine the abundance of non-primordial Helium, we have to consider stellar models. That is because, usually, stars presenting He-emission lines aren't particularly trustworthy due to sedimentation of Helium, which essentially causes the superficial abundance not to reflect the original one.
...
\end{equation}
which can be inverted to obtain an expression of the form $\log_{10}(\text{Z}_{*}) \sim [\text{Fe}\,|\,\text{H}]^{-1.85}_{-1.77}$, where we're assuming $\text{Y}\sim\text{Y}_{\odot}\sim 0.24$. The subscript comes from the estimate of \cite{2022A&A...661A.140M} (Magg et al, 2022), while the superscript comes from the estimate of \cite{2021A&A...653A.141A} (Asplund et al, 2021) of the solar metallicity.
