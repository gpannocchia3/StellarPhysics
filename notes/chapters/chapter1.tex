...
\begin{figure}[h!]
	...
	\label{fgr:solarbb}
\end{figure}
\par Although a tad annoying for the characterization of the radiation field, this deviation from ideal behavior allows us to infer the chemical composition of stars!
...
\end{figure}
\par Since elements are (on avergae) produced essentially via the same producedures, we may expect to measure a constant ratio between the various abundances of elements. Consider for example Fig.\ref{fgr:universalcurveabundances}.
...
\end{equation}
which can be inverted to obtain an expression of the form $\log_{10}(\text{Z}_{*}) \sim [\text{Fe}\,|\,\text{H}]^{-1.85}_{-1.77}$, where we're assuming $\text{Y}\sim\text{Y}_{\odot}\approx 0.24$. The subscript comes from the estimate of \cite{2022A&A...661A.140M} (Magg et al, 2022), while the superscript comes from the estimate of \cite{2021A&A...653A.141A} (Asplund et al, 2021) of the solar metallicity.
...
\begin{itemize}
	...
	\item \emph{PopIII Stars}: Allegedly, this population of star should be the first one to form after the Big Bang. As so, its metallicity content should be virtually zero. This would imply that they should have large masses\footnote{In fact, they should be massive enough to leave behind, after their death, an intermediate mass black hole.} and be very bright. As you might imagine, PopIII star wouldn't live for long.
\end{itemize} 
